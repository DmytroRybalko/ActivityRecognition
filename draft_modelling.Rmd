...
```{r}
importance(random.forest, type = 2)
```
...
Now, let's create our training and testing sets using the subset function.
```{r}
Train <- subset(raw_features, spl == TRUE)
Test <- subset(raw_features, spl == FALSE)
```
...
```{r}
base.CART <- rpart(user_id ~ ., data = Train, method = "class", minbucket = 25)
```
...
Now let's see how well our CART model does at making predictions for the test set.
```{r}
PredictCART <- predict(base.CART, newdata = Test, type = "class")
```
Now let's compute the accuracy of our model by building a confusion matrix.
```{r}
confusionMatrix(Test$user_id %>% as.factor(), PredictCART %>% as.factor())
```
