...
```{r}
cm <- confusionMatrix(reference = trainset$Species, # actual data
                data = predict.train,               # predicted data
...
confusionMatrix(reference = trainset$Species, 
                ...
                prevalence = c("setosa" = 0.5, "versicolor" = 0.25, " virginica" = 0.25))
```
...
```{r}
PredictCART <- predict(iris.CART, newdata = testset, type = "class")
```
Now let's compute the accuracy of our model by building a confusion matrix.
```{r}
confusionMatrix(reference = testset$Species, data = PredictCART,
                mode = "prec_recall",               
                dnn = c("Reference", "Prediction"))
```
